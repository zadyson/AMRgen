  ...
  for (sample in seq(1, length(ids), by = batch_size)) {
    ...
    for (record in 1:length(data_list)){
      ...
      for (entry in 1:length(data_entries)){
        ...
        bioproj <- pluck(data_list, 1, "BioSample", "Links", 2, "Link", ".attrs", 3, .default = NA)
        
        if (is.na(bioproj)){
          bioproj <- pluck(data_list, "BioSample", "Links", "Link", ".attrs", 3, .default = NA)
        }
        
        # Add accessions and organism cols
        temp_entry <- temp_entry %>%
    ...
    # pause between record batch pulls
