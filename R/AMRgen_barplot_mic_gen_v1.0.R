...
#' @param pheno_data A data.frame read in by the phenotypic parser, containing a column listing the sample ID, "spp_pheno" (class mo),
#' "drug_agent" (class ab), "mic" (class mic), "disk" (class disk).
#' The sample ID column is used to match to geno_data.
#' @param geno_data A data.frame generated by the genotypic parser, containing a sample ID column, "marker" (class gene),
...
#' 
#' @param abs_to_plot A vector of strings listing the antibiotic drugs you wish to plot. Need to be
...
AMRgen_barplot_mic_gen <- function(pheno_data, geno_data,
  ...
  if(is.null(geno_sample_col)){geno_sample_col <- colnames(geno_data)[1]}
  if(is.null(pheno_sample_col)){pheno_sample_col <- colnames(pheno_data)[1]}
