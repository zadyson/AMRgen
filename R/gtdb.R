...
#'
#' @param file File path to GTDB output file (tab-separated). If not given, tbl must be given.
#' @param tbl Data frame containing GTDB output. If not given, file must be given.
#' @param species_column Name of column containing the species call (default "Species").
#'
#' @return tbl Data frame containing GTDB output with AMR-parsed microorganism code (gtdb.mo) and species name (gtdb.species) appended.
#' @export
#'
#' @examples import_gtdb(tbl = data.frame(Species = c("Pseudomonas_E piscis","Haemophilus_D parainfluenzae_A", "Acinetobacter calcoaceticus_C")))
#' 
import_gtdb <- function(file=NULL, tbl=NULL, species_column="Species") {
  if(!is.null(file)) {
    tbl <- read_tsv(file)
  }
  tbl %>% mutate(gtdb.mo=gtdb.mo(tbl[[species_column]]), gtdb.species=mo_name(gtdb.mo, keep_synonyms = TRUE))
}
