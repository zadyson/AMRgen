...
``` {r assay_by_genotype}
assay_by_var(cip_bin %>% mutate(drug_agent="CIP"), measure="mic", colour_by="parC_S80I", antibiotic="CIP")$plot

# count the number of gyrA mutations per genome, and colour by that count
gyrA_mut <- cip_bin %>% 
  mutate(gyrA_mut = rowSums(across(contains("gyrA_") & where(is.numeric)), na.rm=T)) %>% 
  select(mic, gyrA_mut)

mic_by_gyrA_count <- assay_by_var(gyrA_mut %>% mutate(drug_agent="CIP"), measure="mic", colour_by="gyrA_mut", antibiotic="CIP")
...
mic_by_marker_count <- assay_by_var(marker_count %>% mutate(drug_agent="CIP"), measure="mic", colour_by="marker_count", antibiotic="CIP", cols=viridisLite::viridis(max(marker_count$marker_count)+1))
